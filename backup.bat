...
%~d0
CD %~dp0
REM Формирование имени файла резервной копии и имени файла отчёта
SET DATETIME=%DATE:~6,4%-%DATE:~3,2%-%DATE:~0,2% %TIME:~0,2%-%TIME:~3,2%-%TIME:~6,2%
SET DUMPFILE=%PGDATABASE% %DATETIME%.backup
SET LOGFILE=%PGDATABASE% %DATETIME%.log
SET DUMPPATH="Backup\%DUMPFILE%"
SET LOGPATH="Backup\%LOGFILE%"
REM Создание папки для бекапов, если она не существует
IF NOT EXIST Backup MD Backup
IF NOT EXIST %DUMPFILE% MD %DUMPFILE%
IF NOT EXIST %LOGFILE% MD %LOGFILE%
REM Создание резервной копии
CALL "%PGBIN%\pg_dump.exe" --no-password --format=custom --verbose --file=%DUMPPATH% 2>%LOGPATH%
